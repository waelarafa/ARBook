...
public class QRScanner : MonoBehaviour
{
    ...
    [Header("── DEBUG FAKE SCAN ──")]
    [Tooltip("Active le fake scan — simule book_001 sans caméra")]
    public bool fakeScan = true;
    [Tooltip("ID du livre à simuler")]
    public string fakeBookId = "book_001";
    ...
    {
        if (arSession != null) arSession.SetActive(active);
        if (xrOrigin  != null) xrOrigin.SetActive(active);
    }
    ...
    public void EnableScanner()
    {
        ...
        // ✅ FAKE SCAN : simule un scan immédiat sans caméra
        if (fakeScan)
        {
            Debug.Log($"[QRScanner] 🧪 Fake scan activé → {fakeBookId}");
            StartCoroutine(FakeScanCoroutine());
            return;
        }
    ...
    private IEnumerator FakeScanCoroutine()
    {
        // Petite pause pour simuler le temps de scan
        SetStatus("📷 Scan en cours...");
        yield return new WaitForSeconds(1f);

        Debug.Log($"[QRScanner] ✅ Fake QR détecté : {fakeBookId}");
        StartCoroutine(HandleBookId(fakeBookId));
    }
    ...
    // ══════════════════════════════════════════════════════════
    // RÉCEPTION FRAME (ignorée si fakeScan)
    // ══════════════════════════════════════════════════════════
    void OnCameraFrameReceived(ARCameraFrameEventArgs args)
    {
        if (!_scanActive) return;
        if (fakeScan) return; // fake scan gère tout
        ...
        finally { image.Dispose(); _isProcessing = false; }
    }
    ...
    private IEnumerator HandleBookId(string bookId)
    {
        ...
        {
            Debug.Log($"[QRScanner] 📦 {bookId} déjà en cache — téléchargement ignoré.");
        }
        else
        {
            float timer = 0f;
